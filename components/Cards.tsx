...
const Cards = ({ titel, body, img, height, width, children }: CardProps) => {
  return (
    <div style={{}} className="w-full sm:w-2/4 md:w-1/3 h-full px-6 py-6 ">
      {/* かーど transition duration-150 ease-in-out transform hover:scale-125 bg-blue-500 text-white font-bold py-2 px-4 rounded */}
      <div
        style={{
          height: "100%",
        }}
        className=" select-none rounded overflow-hidden h-full shadow hover:shadow-xl my-2 duration-300 transition  ease-in-out transform hover:scale-105"
      >
        {(() => {
          if (height && width) {
            return (
